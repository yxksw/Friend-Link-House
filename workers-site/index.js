...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      } else if (path === "/api/posts/latest" || path === "/api/posts/latest/") {
...
async function initDatabase(db, corsHeaders) {
  try {
    ...
    // 创建索引
    await db.prepare(`CREATE INDEX IF NOT EXISTS idx_posts_blog_id ON posts(blog_id)`).run();
    await db.prepare(`CREATE INDEX IF NOT EXISTS idx_posts_pub_date ON posts(pub_date DESC)`).run();
    await db.prepare(`CREATE INDEX IF NOT EXISTS idx_blogs_category ON blogs(category)`).run();
    await db.prepare(`CREATE INDEX IF NOT EXISTS idx_blogs_status ON blogs(status)`).run();
